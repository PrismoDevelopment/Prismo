...
module.exports = class Noprefix extends Command {
    ...
    async run({ message, args }) {
        ...
        const data = await this.client.database.noprefixUserData.get(this.client.user.id);
        if (!args[0]) {
        ...
        if (args[0] == "add") {
            ...
            this.client.database.noprefixUserData.put(this.client.user.id, data);
        ...
        if (args[0] == "list") {
            ...
            collector.on("collect", async (interaction) => {
                ...
                if (interaction?.customId === backId) {
                    page--;
                    if (page < 1) page = 1;
                    embed.setDescription(queue.slice((page - 1) * 10, page * 10).join("\n"));
                    embed.setFooter({
                        text: `Page ${page} of ${Math.ceil(data.userids.length / 10)}`,
                    });
                    await interaction?.update({ embeds: [embed] });
                }
