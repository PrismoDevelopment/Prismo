    ...
    async run({ message, args }) {
        try {
            ...
            if (args[0] === "set") {
                ...
                if (check) return;
                await this.client.database.guildData.putVcrole(message?.guild.id, role.id);
                return message?.reply({
                ...
                if (!data) {
                    data = await this.client.database.guildData.get(message?.guild?.id);
                    if (data != null)
    ...
    async exec({ interaction }) {
        try {
            ...
            if (interaction?.options?.getSubcommand() === "set") {
                ...
                if (check) return;
                await this.client.database.guildData.putVcrole(interaction?.guild.id, role.id);
                return interaction?.reply({
                    content: `Successfully Set ${role} As Vc Role!`,
                    ephemeral: true,
                });
                ...
                if (!data) {
                    data = await this.client.database.guildData.get(interaction?.guild?.id);
                    if (data != null)
