...
module.exports = class Dare extends Command {
    ...
    async run({ message, args }) {
        ...
        if (!emoji.id)
            return message?.reply({
                embeds: [
                    this.client.util
                        .embed()
                        .setDescription(`The reaction must be a custom emoji.`)
                        .setColor(this.client.config.Client.ErrorColor),
                ],
            });
        if (!time)
        ...
        if (isNaN(ms(time)) || !ms(time))
            return message?.reply({
                embeds: [
                    this.client.util
                        .embed()
                        .setDescription(`The time must be a number.`)
                        .setColor(this.client.config.Client.ErrorColor),
                ],
            });
        if (isNaN(winners))
            return message?.reply({
                embeds: [
                    this.client.util
                        .embed()
                        .setDescription(`The winners must be a number.`)
                        .setColor(this.client.config.Client.ErrorColor),
                ],
            });
        if (winners > 10)
