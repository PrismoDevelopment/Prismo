...
module.exports = class Util {
    ...
    parseEmoji(emoji) {
        ...
        if (emoji instanceof Object) return emoji.id ? `${emoji.name}:${emoji.id}` : emoji.name;
        if (typeof emoji === "string") {
            if (emoji.includes("%")) emoji = decodeURIComponent(emoji);
            if (emoji.includes(":")) {
                const parsedEmoji = Util.parseEmoji(emoji.split(":"));
                if (parsedEmoji instanceof Array) return parsedEmoji.map((x) => Util.parseEmoji(x));
                return parsedEmoji;
            } else {
    ...
    color(message) {
        const rang =
            message.guild.members.cache.get(this.client.user.id).displayHexColor === "#000000"
    ...
    async replacerString(objectValue, message) {
        ...
        if (objectValue.title != null) {
            ...
            objectValue.title = objectValue.title.replace(/\$user_id/g, message.member.user.id);
            objectValue.title = objectValue.title.replace(
                /\$user_createdtimestamp/g,
                `<t:${~~(message.member.user.createdTimestamp / 1000)}:R>`
            );
            objectValue.title = objectValue.title.replace(/\$guild_name/g, message.guild.name);
            objectValue.title = objectValue.title.replace(/\$guild_id/g, message.guild.id);
            objectValue.title = objectValue.title.replace(
                /\$guild_membercount/g,
                message.guild.memberCount
            );
    ...
    async replacerOriginal(objectValue, member) {
        ...
        if (objectValue.description != null) {
            ...
            objectValue.description = objectValue.description.replace(/\$user_id/g, member.user.id);
            objectValue.description = objectValue.description.replace(
                /\$user_createdtimestamp/g,
                member.user.createdTimestamp
            );
        ...
        if (objectValue.title != null) {
            objectValue.title = objectValue.title.replace(/\$user_mention/g, `<@${member.id}>`);
            objectValue.title = objectValue.title.replace(/\$user_username/g, member.user.username);
            objectValue.title = objectValue.title.replace(/\$user_tag/g, member.user.username);
            objectValue.title = objectValue.title.replace(/\$user_id/g, member.user.id);
            objectValue.title = objectValue.title.replace(
                /\$user_createdtimestamp/g,
                member.user.createdTimestamp
            );
            objectValue.title = objectValue.title.replace(/\$guild_name/g, member.guild.name);
            objectValue.title = objectValue.title.replace(/\$guild_id/g, member.guild.id);
            objectValue.title = objectValue.title.replace(
                /\$guild_membercount/g,
                member.guild.memberCount
            );
    ...
    async replace(string, message) {
        try {
            ...
            string = await string.replace(/\$user_mention/g, `<@${message.member.id}>`);
            string = await string.replace(/\$user_username/g, message.member.user.username);
            string = await string.replace(/\$user_tag/g, message.member.user.username);
            ...
            string = await string.replace(/\$guild_membercount/g, message.guild.memberCount);
            return string;
        } catch (e) {
            return string;
        }
    ...
    async replaceOriginal(string, member) {
        try {
            ...
            string = await string.replace(/\$guild_membercount/g, member.guild.memberCount);
            return string;
        } catch (e) {
            console.error(e);
        }
    ...
    */
    ...
    async rolePerms(role) {
        ...
        const check = new PermissionsBitField(role.permissions).any(permissionNames);
        return check;
    }
