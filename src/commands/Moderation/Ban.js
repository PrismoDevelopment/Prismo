...
module.exports = class help extends Command {
    ...
    async run({ message, args }) {
        ...
        if (!args)
            return message?.reply({ content: "Please provide a user to ban!" });
        ...
        if (membercheckowner)
            return message?.reply({ content: "You can't ban the bot owner!" });
        if (owner) {
        ...
        if (!owner) {
            ...
            if (
                ...
                return message?.reply({
                    content:
                        "You can't ban a user with same or higher roles as you!",
                });
            }
        }
        if (!member.bannable)
            return message?.reply({ content: "I can't ban that user!" });
    ...
    async exec({ interaction }) {
        ...
        const reason =
            interaction?.options.getString("reason") + " | Banned By: " + interaction?.user.username || "No reason provided | Banned By: " + interaction?.user.username;
        await member.ban({ days: 7, reason: reason });
        const embed = this.client.util
            .embed()
            .setDescription(`**${member.user.username}** has been banned!`)
