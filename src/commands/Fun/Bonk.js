...
module.exports = class Bonk extends Command {
    ...
    async run({ message, args }) {
        ...
        if (member.id === message?.author.id) return message?.reply("You can't bonk yourself!");

        if (member.id === this.client.user.id) return message?.reply("You can't bonk me!");

        const bonk = await discordActions.bonk();
    ...
    async exec({ interaction }) {
        ...
        if (member.id === this.client.user.id) return interaction?.reply("You can't bonk me!");
