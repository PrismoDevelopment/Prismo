...
module.exports = class help extends Command {
    ...
    async run({ message, args }) {
        ...
        if (!timeArg) {
            ...
            else if (args[1].endsWith("s") || args[1].endsWith("sec") || args[1].endsWith("second"))
                timeArg *= 1;
            else if (args[1].endsWith("m") || args[1].endsWith("min") || args[1].endsWith("minute"))
                timeArg *= 60;
            else if (args[1].endsWith("h") || args[1].endsWith("hr") || args[1].endsWith("hour"))
                timeArg *= 60 * 60;
        ...
        if (!reason) return message?.reply({ content: "Please provide a valid reason." });
        ...
        if (membercheckowner) return message?.reply({ content: "You can't mute the bot owner!" });
        if (owner) {
        ...
        if (!owner && message.author.id !== message.guild.ownerId) {
            if (member.roles.highest.position >= message?.member.roles.highest.position)
                return message?.reply({
                    content: "You can't timeout a user with same or higher roles as you!",
                });
            if (
        ...
        if (!member.manageable) return message?.reply({ content: "I can't timeout that user!" });
    ...
    async exec({ interaction }) {
        ...
        if (timeArg) {
            timeArg = timeArg.replace(/\D/g, "");
            if (timeArg === "") timeArg = 600;
            else if (timeArg.endsWith("s") || timeArg.endsWith("sec") || timeArg.endsWith("second"))
                timeArg *= 1;
            else if (timeArg.endsWith("m") || timeArg.endsWith("min") || timeArg.endsWith("minute"))
                timeArg *= 60;
            else if (timeArg.endsWith("h") || timeArg.endsWith("hr") || timeArg.endsWith("hour"))
                timeArg *= 60 * 60;
