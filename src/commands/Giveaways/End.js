...
module.exports = class Dare extends Command {
    ...
    async run({ message, args }) {
        const hasPerm =
            ...
            message?.member.roles.cache.some((r) => r.name.toLowerCase() === "giveaways") ||
            message?.guild.ownerId === message?.author.id ||
            this.client.util.checkOwner(message?.author.id) ||
            message?.member.roles.cache.some((r) => r.name.toLowerCase() === "giveaway");
        if (!hasPerm)
            ...
            if (!id) {
                ...
                msg = msg.filter(
                    (m) =>
                        ...
                        m.content.includes(`${this.client.config.Client.emoji.gift}`)
                );
                msg = msg.first();
                if (!msg)
                    return message?.reply({
                        embeds: [
                            this.client.util.errorDelete(
                                message,
                                `I could not find a giveaway with that ID.`
                            ),
                        ],
                    });
                id = msg.id;
            ...
            return message?.reply({
                embeds: [
                    this.client.util
                        .embed()
                        .setDescription(
                            `You have missed an option \`id\`. Example:\n\`\`\`yml\n${message?.guild.config.prefix}gend <id>\n\`\`\``
                        )
                        .setColor(this.client.config.Client.ErrorColor),
                ],
            });
        ...
        if (giveaway.ended)
            return message?.reply({
                embeds: [this.client.util.errorDelete(message, `This giveaway has already ended.`)],
            });
        giveaway.end();
    ...
    async exec({ interaction }) {
        ...
        if (giveaway.ended)
            return interaction?.reply({
                embeds: [
                    this.client.util.errorDelete(interaction, `This giveaway has already ended.`),
                ],
            });
        giveaway.end();
