...
module.exports = class notificatiom extends Command {
    constructor(...args) {
        super(...args, {
            ...
            category: "Utilities",
            examples: [
                "ytnotifier https://www.youtube.com/channel/UCpEhnqL0y41EpW2TvWAHD7Q #general true",
            ],
    ...
    async run({ message, args }) {
        ...
        let msg = await message.channel.send({
            content: `Youtube notifications are currently ${guildData?.ytchannel ? "enabled" : "disabled"} for this server.`,
        ...
        collector.on("collect", async (button) => {
            if (button.customId === "enable") {
                ...
                let nuinuimsg = await message.channel.send({
                    content: "Please provide the youtube channel url.",
                });
                ...
                await new Promise((resolve) => {
                    nuinuicollector.on("collect", async (m) => {
                        if (m.content.toLowerCase() === "cancel")
                            return nuinuicollector.stop("cancelled");
                        let channelid;
                        ...
                        request
                            ...
                            .then(async (data) => {
                                ...
                                if (guildData.postedvids.includes(id))
                                    return nuinuicollector.stop("already");
                ...
                nuinuicollector.on("end", async (collected, reason) => {
                    if (reason === "time")
                        return nuinuimsg.edit({ content: "You took too long to respond." });
                });

                let nuinuimsg2 = await message.channel.send({
                    content: "Please provide the channel to send notifications in.",
                });
                ...
                await new Promise((resolve) => {
                    nuinuicollector2.on("collect", async (m) => {
                        if (m.content.toLowerCase() === "cancel")
                            return nuinuicollector2.stop("cancelled");
                        let channel =
                ...
                nuinuicollector2.on("end", async (collected, reason) => {
                    if (reason === "time")
                        return nuinuimsg2.edit({ content: "You took too long to respond." });
                    if (reason === "invalid")
                        return nuinuimsg2.edit({ content: "Invalid channel provided." });
                });
                let nuinuimsg3 = await message.channel.send({
                    content: "Please provide whether to ping everyone or not.",
                    components: [
                        {
                            type: 1,
                            components: [
                                { type: 2, label: "Yes", style: 3, custom_id: "yes" },
                                { type: 2, label: "No", style: 4, custom_id: "no" },
                            ],
                        },
                    ],
                });
                ...
                nuinuicollector3.on("collect", async (button) => {
                    if (button.customId === "yes") {
                        ...
                        msg.edit({
                            content: "Youtube notifications have been enabled for this server.",
                            components: [],
                        });
                ...
                nuinuicollector3.on("end", async (collected, reason) => {
                    if (reason === "time")
                        return nuinuimsg3.edit({ content: "You took too long to respond." });
                    nuinuimsg3.edit({
                        content: "Youtube notifications have been enabled for this server.",
                    });
                });
                ...
                await this.client.database.ytData.post(message.guild.id, guildData);
                msg.edit({
                    content: "Youtube notifications have been disabled for this server.",
                    components: [],
                });
            }
        });
        ...
        if (subcommand === "enable") {
            if (guildData.ytchannel)
                return interaction.reply({
                    content: "Youtube notifications are already enabled for this server.",
                    ephemeral: true,
                });
            ...
            if (!guildData.ytchannel)
                return interaction.reply({
                    content: "Youtube notifications are already disabled for this server.",
                    ephemeral: true,
                });
            guildData.ytchannel = null;
            guildData.channel = null;
