    ...
    async add(message, user, role, slash = false) {
        ...
        if (user._roles.includes(role.id)) {
            await user.roles
                .remove(
                    role.id,
                    `Removed By ${message.member.user.tag} | ${this.client.user.username}`
                )
                .then(() => {
                    message.reply({
                        content: `**${user.user.tag}** Has Been Removed From **${role.name}** Role.`,
                        ephemeral: true,
                    });
                })
                .catch((e) => {
            ...
            await user.roles
                .add(
                    role.id,
                    `Added By ${message.member.user.tag} | ${this.client.user.username}`
                )
                .then(() => {
                    message.reply({
                        content: `**${user.user.tag}** Has Been Added To **${role.name}** Role.`,
                        ephemeral: true,
                    });
                })
                .catch((e) => {
