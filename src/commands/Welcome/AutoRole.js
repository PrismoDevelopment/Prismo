        ...
        try {
            ...
            if (args[0].toLowerCase() === "add" || args[0].toLowerCase() === "set") {
                ...
                if (args[1].toLowerCase() === "bot") {
                    ...
                    if (perms)
                        return message?.reply({
                            embeds: [
                                this.client.util.errorDelete(
                                    message,
                                    "You Can't Add This Role To Autorole As This Role Has Administrator Permissions"
                                ),
                            ],
                            ephemeral: true,
                        });
                    ...
                    if (data.autorole.botRoles.includes(role.id))
                        return message?.reply({
                            embeds: [
                                this.client.util.errorDelete(
                                    message,
                                    "This Role Is Already Added To Bot Autorole"
                                ),
                            ],
                            ephemeral: true,
                        });
                    if (data.autorole.botRoles.length > 2) {
                        return message?.reply({
                            embeds: [
                                this.client.util.errorDelete(
                                    message,
                                    "You Can't Add More Than 3 Roles To Bot Autorole"
                                ),
                            ],
                            ephemeral: true,
                        });
                    ...
                    if (data.autorole.botRoles.length > 0 || data.autorole.humanRoles.length > 0)
                        data.autorole.enabled = true;
                    this.client.database.guildData.putAutorole(message?.guild.id, data.autorole);
                    return message?.reply({
                        embeds: [
                            this.client.util.doDeletesend(message, `Added ${role} To Bot Autorole`),
                        ],
                        ephemeral: true,
                    });
                    ...
                    if (data.autorole.humanRoles.includes(role.id))
                        return message?.reply({
                            embeds: [
                                this.client.util.errorDelete(
                                    message,
                                    "This Role Is Already Added To Human Autorole"
                                ),
                            ],
                            ephemeral: true,
                        });
                    if (data.autorole.humanRoles.length > 2) {
                        return message?.reply({
                            embeds: [
                                this.client.util.errorDelete(
                                    message,
                                    "You Can't Add More Than 3 Roles To Human Autorole"
                                ),
                            ],
                            ephemeral: true,
                        });
                    ...
                    message?.reply({
                        embeds: [
                            this.client.util.errorDelete(
                                message,
                                "You Must Specify Whether You Want To Add Role To Bot Autorole Or Human Autorole"
                            ),
                        ],
                        ephemeral: true,
                    });
            ...
            if (args[0].toLowerCase() === "remove" || args[0].toLowerCase() === "delete") {
                ...
                if (!role)
                    return message?.reply({
                        embeds: [
                            this.client.util.errorDelete(message, "You Must Mention A Valid Role"),
                        ],
                        ephemeral: true,
                    });
                const data = await this.client.database.guildData.get(message?.guild.id);
                if (
                    ...
                    return message?.reply({
                        embeds: [
                            this.client.util.errorDelete(
                                message,
                                "This Role Is Not Added To Autorole"
                            ),
                        ],
                        ephemeral: true,
                    });
                data.autorole.botRoles = data.autorole.botRoles.filter((r) => r !== role.id);
                data.autorole.humanRoles = data.autorole.humanRoles.filter((r) => r !== role.id);
                if (data.autorole.botRoles.length == 0 || data.autorole.humanRoles.length == 0)
                    data.autorole.enabled = false;
                this.client.database.guildData.putAutorole(message?.guild.id, data.autorole);
                return message?.reply({
                    embeds: [
                        this.client.util.doDeletesend(message, `Removed ${role} From Autorole`),
                    ],
                    ephemeral: true,
                });
            }
                ...
                const embed = this.client.util
                    ...
                    .setDescription(
                        `**Autorole Bot Roles List:**\n${
                            botRoles.length > 0 ? botRoles : "No Roles In Autorole"
                        }\n\n**Autorole Human Roles List:**\n${
                            humanRoles.length > 0 ? humanRoles : "No Roles In Autorole"
                        }`
                    )
    ...
    async exec({ interaction }) {
        try {
            ...
            if (args === "remove") {
                ...
                if (data.autorole.botRoles.length == 0 || data.autorole.humanRoles.length == 0)
                    data.autorole.enabled = false;
                this.client.database.guildData.putAutorole(interaction?.guild.id, data.autorole);
                return interaction?.reply({
