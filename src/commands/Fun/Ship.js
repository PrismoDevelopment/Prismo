...
module.exports = class Ship extends Command {
    ...
    async run({ message, args }) {
        ...
        if (!member) {
            let members = await message?.guild.members
                ...
                .then((members) =>
                    members
                        .filter(
                            (member) => !member.user.bot && member.user.id !== message?.author.id
                        )
        ...
        if (user1.id === user2.id)
            return message?.channel.send("You can't ship the same user twice!");
        if (user1.id === this.client.user.id)
            return message?.channel.send(
                "I'm flattered, but I'm already in a relationship with my developer!"
            );
        ...
        if (love >= 50) {
            embed.setDescription(
                `${user1.username} and ${user2.username} are ${love}% in love! Maybe they should date?`
            );
        } else {
            embed.setDescription(
                `${user1.username} and ${user2.username} are ${love}% in love! Maybe they should just be friends?`
            );
        }
