    ...
    async run({ message, args }) {
        ...
        if (args[0].toLowerCase() == "enable") {
            ...
            const embed = this.client.util
                ...
                .setTitle("Anti Nuke")
                .setDescription(
                    `${this.client.config.Client.emoji.tick} | Anti Nuke has been **enabled**.`
                )
                .setColor(this.client.PrimaryColor)
                .setTimestamp();
            return message.channel.send({ embeds: [embed] });
        ...
        if (args[0].toLowerCase() == "disable") {
            if (!antiNukeData.enabled) {
                const embed = this.client.util
                    ...
                    .setTitle("Anti Nuke")
                    .setDescription(
                        `${this.client.config.Client.emoji.tick} | Anti Nuke is already **disabled**.`
                    )
                    .setColor(this.client.PrimaryColor)
                    .setTimestamp();
                return message.channel.send({ embeds: [embed] });
            ...
            const embed = this.client.util
                ...
                .setTitle("Anti Nuke")
                .setDescription(
                    `${this.client.config.Client.emoji.tick} | Anti Nuke has been **disabled**.`
                )
                .setColor(this.client.PrimaryColor)
                .setTimestamp();
            return message.channel.send({ embeds: [embed] });
        ...
        if (interaction.options.getSubcommand() == "enable") {
            if (antiNukeData.enabled) {
                const embed = this.client.util
                    ...
                    .setTitle("Antinuke")
                    .setDescription(
                        `${this.client.config.Client.emoji.tick} | Anti Nuke is already **enabled**.`
                    )
                    .setColor(this.client.PrimaryColor)
                    .setTimestamp();
                return interaction.reply({ embeds: [embed] });
