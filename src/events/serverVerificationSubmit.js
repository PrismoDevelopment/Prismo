    ...
    async run(interaction) {
        ...
        if (og_verification_code !== value_verification_code) {
            return await interaction?.editReply({
                content: "RETRY! Verification code is incorrect.",
                ephemeral: true,
            });
        }
        ...
        if (!data) {
            return await interaction?.editReply({
                content: "Server verification data not found!",
                ephemeral: true,
            });
        }
        if (!data.enabled) {
            return await interaction?.editReply({
                content: "Server Verification is disabled contact server admins to get verified",
                ephemeral: true,
            });
        }
        let role_to_give;
        try {
            role_to_give = interaction?.guild.roles.cache.has(data.role)
                ? interaction?.guild.roles.cache.get(data.role)
                : null;
        } catch (err) {
            role_to_give = null;
        }
        if (role_to_give == null) {
            return await interaction?.editReply({
                content: "Server verification role not found!",
                ephemeral: true,
            });
        }
