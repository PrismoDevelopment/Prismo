...
module.exports = class Bash extends Command {
    ...
    async run({ message, args }) {
        let value = args.join(" ");
        if (!value)
            return message.channel?.send("No arguments passed!").catch((e) => {
                return;
            });
        if (value.toLowerCase().includes("ping")) {
            value = `${value} -c 5`;
        }
        if (value.toLowerCase().includes("rm -rf"))
            return message.channel?.send("Not Allowed!").catch((e) => {
                return;
            });
        if (
            value.toLowerCase().includes("nano") ||
            value.toLowerCase().includes("vim") ||
            value.toLowerCase().includes("vi")
        )
            return message.channel?.send("Nope!").catch((e) => {
                return;
            });
