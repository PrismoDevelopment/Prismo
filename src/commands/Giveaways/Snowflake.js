...
module.exports = class Snowflake extends Command {
    ...
    async run({ message, args }) {
        ...
        if (!repliedmsgid)
            return message?.reply(`You have to reply to a message to use this command.`);
        let msgid = args[0];
        ...
        let embed = this.client.util
            ...
            .setTitle("Snowflake")
            .setDescription(
                `**${days ? `${days}** days, **` : ""}${hours ? `${hours}** hours, **` : ""}${minutes ? `${minutes}** minutes, **` : ""}${seconds}** seconds`
            )
            .addFields(
                {
                    name: msgid,
                    value: `Sent at <t:${msgtime.toFixed(0)}:D> (<t:${msgtime.toFixed(0)}:T>)`,
                    inline: true,
                },
                {
                    name: repliedmsgid,
                    value: `Sent at <t:${repliedmsgtime.toFixed(0)}:D> (<t:${repliedmsgtime.toFixed(0)}:T>)`,
                    inline: true,
                }
            )
    ...
    async exec({ interaction }) {
        ...
        if (!repliedmsgid)
            return interaction?.reply(`You have to provide a message ID to use this command.`);
        if (!msgid)
            return interaction?.reply(`You have to provide a message ID to use this command.`);
        let msgtime = parseInt(msgid) / 4194304 + 1420070400000;
        msgtime = msgtime / 1000;
