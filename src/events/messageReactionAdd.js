...
module.exports = class messageReactionAdd extends Event {
    ...
    async run(messageReaction, user) {
        ...
        let starboardChannel = messageReaction.message.guild.channels.cache.get(
            data.starboardChannel
        );
        if (!starboardChannel) return;

        let starboardMessage = await messageReaction.message.channel.messages.fetch(
            `${messageReaction.message.id}`
        );
        if (!starboardMessage) return;
        let starboardEmbed = this.client.util
            .embed()
            .setAuthor({
                name: starboardMessage.author.username,
                iconURL: starboardMessage.author.displayAvatarURL({ dynamic: true }),
            })
        ...
        let compos = [
            {
                type: 1,
                components: [
                    { type: 2, style: 5, label: "Jump to Message", url: starboardMessage.url },
                ],
            },
        ];
        ...
        let starboardmsg = starboardmessages.find(
            (m) => m?.embeds[0]?.footer?.text === "ID: " + starboardMessage.id
        );
        if (starboardmsg) {
            starboardmsg
                .edit({ content: `⭐ **${messageReaction.count}** | ${starboardMessage.url}` })
                .catch(() => {});
        } else {
            starboardChannel
                .send({
                    content: `⭐ **${messageReaction.count}** | ${starboardMessage.url}`,
