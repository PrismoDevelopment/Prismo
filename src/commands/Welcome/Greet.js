...
module.exports = class help extends Command {
    ...
    async run({ message, args }) {
        ...
        if (args[0] === "test") {
            if (!data.greet.enabled)
                return message?.reply(
                    `${this.client.config.Client.emoji.cross} Greet is not enabled!`
                );
            ...
            return message?.reply(
                `${this.client.config.Client.emoji.tick} Greet has been disabled!`
            );
        } else if (args[0] === "list") {
            if (!data.greet.enabled)
                return message?.reply(
            ...
            if (channels.length === 0)
                return message?.reply(
                    `${this.client.config.Client.emoji.cross} No channels are set!`
                );
            let hemuembed = this.client.util
                .embed()
        ...
        if (!channel)
            return message?.reply(
                `${this.client.config.Client.emoji.cross} Please mention a channel!`
            );
        if (data.greet.channel.includes(channel.id)) {
            data.greet.channel = data.greet.channel.filter((c) => c !== channel.id);
            this.client.database.guildData.putGreet(message?.guild.id, data.greet);
            ...
            this.client.database.guildData.putGreet(message?.guild.id, data.greet);
            if (data.greet.channel.length === 3)
                return message?.reply(
                    `${this.client.config.Client.emoji.cross} You can only set greet in 3 channels!`
                );
            if (data.greet.channel.length > 0) {
            ...
            return message?.reply(
                `${this.client.config.Client.emoji.tick} Enabled greet for <#${channel.id}>`
            );
        }
    }
    ...
    async exec({ interaction, args }) {
        ...
        if (data.greet.channel.includes(channel.id)) {
            data.greet.channel = data.greet.channel.filter((c) => c !== channel.id);
            this.client.database.guildData.putGreet(interaction?.guild.id, data.greet);
            if (data.greet.channel.length === 0) {
                data.greet.enabled = false;
                this.client.database.guildData.putGreet(interaction?.guild.id, data.greet);
            }
