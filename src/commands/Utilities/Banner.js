    ...
    async run({ message, args }) {
        ...
        if (!banner)
            return message?.channel.send({
                embeds: [this.client.util.errorDelete(message, "User does not have a banner.")],
            });
        const embed = this.client.util
            .embed()
            .setAuthor({
                name: realUser.username + "'s Banner",
                iconURL: realUser.displayAvatarURL({ dynamic: true }),
            })
            .setImage(banner)
    ...
    async exec({ interaction }) {
        ...
        if (!banner)
            return interaction?.reply({
                embeds: [this.client.util.errorDelete(interaction, "User does not have a banner.")],
                ephemeral: true,
            });
        const embed = this.client.util
            .embed()
