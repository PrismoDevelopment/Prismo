...
module.exports = class Marry extends Command {
    ...
    async run({ message, args }) {
        ...
        if (!args[0] && newdata.married) {
            ...
            let embedd = this.client.util
                .embed()
                .setThumbnail(message?.author.displayAvatarURL({ dynamic: true }))
                .setColor(this.client.config.Client.PrimaryColor)
                .setTitle(
                    `<a:p_heart09:1066327933741969488> **${message?.author.username}** is happily married to **${partner ? partner.username : "Unknown"}**`
                )
                .setDescription(
                    `You both have been married on <t:${Math.floor(convertDateToTimestamp(newdata.marriedAt) / 1000)}:f>. You have spent **${findDifference(newdata.marriedAt)}** together <a:marry_me:1066328564368146494>`
                )
                .setFooter({
                    text: `Requested by ${message?.author.username} at ${new Date().toLocaleTimeString()}`,
        ...
        if (selfdata.married) {
            ...
            message?.reply(
                `You are already married to **${partner ? partner.username : "Unknown"}**.`
            );
            return;
        }
        if (data.married) {
            ...
            message?.reply(
                `**${member.user.username}** is already married to **${partner ? partner.username : "Unknown"}**.`
            );
            return;
        }
        if (data.pendingproposal) {
            ...
            message?.reply(
                `**${member.user.username}** already has a proposal from **${proposer ? proposer.username : "Unknown"}**.`
            );
            return;
        }
        let embed = this.client.util
            ...
            .setColor(this.client.config.Client.PrimaryColor)
            .setTitle(
                `**${member.user.username}** has been proposed to by **${message?.author.username}** <a:ring:1066050452900302949>`
            )
            .setDescription(
                `<a:zZ_inc_satashi_ringlelo:1066052671431258243> To accept the proposal, type \`${serverdata.prefix}am\`, or \`${serverdata.prefix}dm\` to decline the proposal.`
            );
    ...
    async exec({ interaction }) {
        ...
        if (!user && newdata.married) {
            ...
            let embedd = this.client.util
                ...
                .setColor(this.client.config.Client.PrimaryColor)
                .setTitle(
                    `<a:p_heart09:1066327933741969488> **${interaction?.user.username}** is happily married to **${partner ? partner.username : "Unknown"}**`
                )
                .setDescription(
                    `You both have been married on <t:${Math.floor(convertDateToTimestamp(newdata.marriedAt) / 1000)}:f>. You have spent **${findDifference(newdata.marriedAt)}** together <a:marry_me:1066328564368146494>`
                )
                .setFooter({
                    text: `Requested by ${interaction?.user.username} at ${new Date().toLocaleTimeString()}`,
        ...
        let embed = this.client.util
            ...
            .setColor(this.client.config.Client.PrimaryColor)
            .setTitle(
                `**${member.user.username}** has been proposed to by **${interaction?.user.username}** <a:ring:1066050452900302949>`
            )
            .setDescription(
                `<a:zZ_inc_satashi_ringlelo:1066052671431258243> To accept the proposal, type \`${serverdata.prefix}am\`, or \`${serverdata.prefix}dm\` to decline the proposal.`
            );
