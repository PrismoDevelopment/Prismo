...
module.exports = class Record extends Command {
    ...
    async run({ message, args }) {
        try {
            ...
            const filter2 = (i) =>
                i.customId === "stop" &&
                i.member.voice.channel.id ===
                    message?.guild.members.cache.get(this.client.user.id).voice.channel.id;
            const collector2 = nuinuimsg.createMessageComponentCollector({
                filter2,
                time: mins * 60000,
            });
            let timeoutID = setTimeout(async () => {
                let buff = await recorder.getRecordedVoiceAsBuffer(
                    message?.guild.id,
                    "single",
                    mins
                );
    ...
    async exec({ interaction }) {
        try {
            ...
            let nuinuimsg = await interaction?.editReply({
                content: `**Recording Started**`,
                components: stopcomponent,
            });
            const filter2 = (i) =>
                i.customId === "stop" &&
                i.member.voice.channel.id ===
                    interaction?.guild.members.cache.get(this.client.user.id).voice.channel.id;
            const collector2 = nuinuimsg.createMessageComponentCollector({
                filter2,
                time: mins * 60000,
            });
