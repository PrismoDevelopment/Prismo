...
module.exports = class GiveawayManager {
    ...
    async mainEmbed(giveaway) {
        if (giveaway.messages.title)
            giveaway.messages.title = giveaway.messages.title.replace("{prize}", giveaway.prize);
        if (giveaway.messages.description) {
            ...
            giveaway.messages.description = giveaway.messages.description.replace(
                "{timestamp}",
                `${this.client.util.discordFormatTime(giveaway.endsAt, "R")} ${this.client.util.discordFormatTime(giveaway.endsAt, "T")}`
            );
            giveaway.messages.description = giveaway.messages.description.replace(
                "{entries}",
            ...
            giveaway.messages.description = giveaway.messages.description.replace(
                "{winners}",
                giveaway.winners.join(", ").length == 0 ? "None" : giveaway.winners.join(", ")
            );
        }
        const embed = this.client.util
            ...
            .setDescription(
                giveaway.messages.description ||
                    `Ends at ${this.client.util.discordFormatTime(giveaway.endsAt, "R")} ${this.client.util.discordFormatTime(giveaway.endsAt, "T")}\nHosted by <@${giveaway.hoster}>`
            )
            .setColor(
    ...
    async endEmbed(giveaway) {
        ...
        const embed = this.client.util
            ...
            .setDescription(
                giveaway.messages.description ||
                    `Ended ${this.client.util.discordFormatTime(giveaway.endsAt, "R")} ${this.client.util.discordFormatTime(giveaway.endsAt, "T")}\n${giveaway.winners.length == 0 ? "" : `Winners: ${giveaway.winners.map((x) => `<@${x}>`).join(", ")}\n`}Hosted by <@${giveaway.hoster}>`
            )
            .setColor(0x2b2d31)
            .setURL(giveaway?.messages?.url || null)
            .setImage(giveaway?.messages?.image || null)
