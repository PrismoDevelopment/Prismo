...
module.exports = class guildBanAdd extends Event {
    ...
    async run(ban) {
        try {
            ...
            if (
                antiNukeData.whitelistusers.includes(user.id) ||
                this.client.util.checkOwner(user.id) ||
                user.id === this.client.user.id ||
                user.id === ban?.guild.ownerId
            ) {
                return;
            }
            await Promise.all([
                this.client
                    .eventRestrict(
                        ...
                        user?.id,
                        guildId,
                        `Anti Ban | Prismo Antinuke`
                    )
                    .catch((e) => {
                        return;
                    }),
                ban?.guild.bans.remove(ban?.user?.id, "Anti Ban | Prismo Antinuke").catch((e) => {
                    return;
                }),
            ]);
            return;
